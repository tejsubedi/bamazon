...
function runAction() {
    ...
    inquirer.prompt({
        ...
        switch (answer.Options) {
            //if the choice is to view products, display all the products by calling the function displayProducts()
    ...
    function lowInventoryItems() {
        ...
        connection.query(query, function (err, res) {
            ...
            for (var i = 0; i < res.length; i++) {
                lowStockTable.push([res[i].item_id, res[i].product_name, res[i].department_name, res[i].price, res[i].stock_quantity]);
            }
    ...
    function addInventory() {
        ...
        inquirer.prompt([
            ...
            connection.query("Select stock_quantity from products where item_id = " + resp.enterID, function (err, data) {
                ...
                var stockToSet = parseInt(curStock) + parseInt(resp.enterQuantity);
                // Update the products table with the stockToSet variable which is sum of current and the desired increase 
    ...
    function addNewProduct() {
        ...
        inquirer.prompt([
            ...
            var newDeptName = newProduct.deptName;
            var newPrice = newProduct.price;
